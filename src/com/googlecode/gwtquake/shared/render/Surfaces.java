...
public abstract class Surfaces  {
	...
	*/
	...
	public static final IntBuffer temp2 = Lib.newIntBuffer(34 * 34, ByteOrder.LITTLE_ENDIAN);
	...
	static void R_DrawAlphaSurfaces()
	{
		...
		glInterleavedArraysT2F_V3F(Polygons.BYTE_STRIDE, globalPolygonInterleavedBuf, staticBufferId);
	...
	private static void glInterleavedArraysT2F_V3F(int byteStride, FloatBuffer buf, int staticDrawIdV) {
		GlState.gl.glEnableClientState(Gl1Context.GL_TEXTURE_COORD_ARRAY);
		GlState.gl.glVertexAttribPointer(Gl1Context.ARRAY_TEXCOORD_0, 2, Gl1Context.GL_FLOAT, 
		    false, byteStride, 0, buf, staticDrawIdV);
		
		GlState.gl.glEnableClientState(Gl1Context.GL_VERTEX_ARRAY);
//		 gl.glVertexPointer(3, byteStride, buf);
		GlState.gl.glVertexAttribPointer(Gl1Context.ARRAY_POSITION, 3, Gl1Context.GL_FLOAT,
	...
	// direct buffer
	static final IntBuffer temp = Lib.newIntBuffer(128 * 128, ByteOrder.LITTLE_ENDIAN);
	...
	static void R_DrawInlineBModel()
	{
		...
		if ( GlState.gl_flashblend.value == 0 )
		{
			...
			for (int k=0 ; k<GlState.r_newrefdef.num_dlights ; k++)
			{
				...
				DynamicLight.R_MarkLights(lt, 1<<k, GlState.currentmodel.nodes[GlState.currentmodel.firstnode]);
			}
		}
	...
	static void R_DrawBrushModel(EntityType e)
	{
		...
		if (Entities.R_CullBox(mins, maxs)) return;
...
//		gl.glTexCoordPointer(2, Polygon.BYTE_STRIDE, globalPolygonTexCoord1Buf);
		GlState.gl.glEnableClientState(Gl1Context.GL_TEXTURE_COORD_ARRAY);
		GlState.gl.glVertexAttribPointer(Gl1Context.ARRAY_TEXCOORD_1, 2, Gl1Context.GL_FLOAT, false, 
				Polygons.BYTE_STRIDE, 20, globalPolygonInterleavedBuf, staticBufferId);
...
//		glInterleavedArraysT2F_V3F(Polygon.BYTE_STRIDE, globalPolygonInterleavedBuf);
		...
		Images.GL_SelectTexture( GlState.GL_TEXTURE1);
		GlState.gl.glEnableClientState(Gl1Context.GL_TEXTURE_COORD_ARRAY);
		GlState.gl.glVertexAttribPointer(Gl1Context.ARRAY_TEXCOORD_1, 2, Gl1Context.GL_FLOAT, 
		    false, Polygons.BYTE_STRIDE, 20, globalPolygonInterleavedBuf, staticBufferId);
//		gl.glTexCoordPointer(2, Polygon.BYTE_STRIDE, globalPolygonTexCoord1Buf);
	...
	*/
	...
	static void LM_UploadBlock( boolean dynamic )
	{
		...
		if ( dynamic )
		{
			...
			GlState.gl.glTexSubImage2D( Gl1Context.GL_TEXTURE_2D, 
							  0,
							  0, 0,
							  BLOCK_WIDTH, height,
							  GL_LIGHTMAP_FORMAT,
